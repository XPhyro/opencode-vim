    ...
    event.subscribe((event, { directory, workspace }) => {
      switch (event.type) {
        ...
        case "message.updated": {
          ...
          // If the bottom of the window has been evicted, drop messages that
          // arrive past our visible tail. They will be loaded on demand.
      ...
      session: {
        ...
        trimNewerMessages(sessionID: string, cap: number) {
          ...
          while (target < messages.length) {
            ...
            const hasInflight = tail.some((message) => message.role === "assistant" && !message.time?.completed)
          ...
          setStore(
            produce((draft) => {
              ...
              for (const message of evicted) delete draft.part[message.id]
              arr.length = target
